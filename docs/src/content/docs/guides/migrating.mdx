...
10. **Sizes follow the container.** `aspectRatio` is an alternative to `height`. DonutChart, Gauge, RadarChart and Sparkline fill their container's width when you omit `size`/`width`. GeoChart loses `width` and fills its container (16:9 by default, or `height`/`aspectRatio`). Gauge and LinearGauge gain `min`.
