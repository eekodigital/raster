...
| Escape            | Clear the selection. Handled inside the chart only, so a surrounding dialog stays open. |
...
- **Forced colours** (Windows High Contrast): marks switch to system colours; series are told apart by dash pattern and marker shape; focus and selection use `Highlight`.
